...
void MultiMLMREML::VectorizeTraits(const double *traits) {
...
double MultiMLMREML::CalcLikelihood() {
  auto *v = vars_;
...
void MultiMLMREML::UpdateGradients() {
...
void MultiMLMREML::UpdateHessian() {
...
MultiMLMREML::MultiMLMREML(const double *lambda, const double *covariates,
...
MultiMLMREML::~MultiMLMREML() noexcept {
...
double MultiMLMREML::CalcLineGradient() {
  double result = 0.0;
...
void MultiMLMREML::CalcAIStep() {
  for (size_t i = 0; i < num_para_dims_; i++) {
    step_[i] = -gradients_[i];
  }
  LAPACKE_dposv(LAPACK_COL_MAJOR, 'L', d, 1, hessian_, d, step_, d);
}
void MultiMLMREML::CalcInitialGuess(double *traits, double *res, double *vars) {
...
void MultiMLMREML::BackupVars() {
  std::copy(vars_, vars_ + num_para_dims_, old_vars_);
}
void MultiMLMREML::UpdateVars(double a) {
  for (size_t i = 0; i < num_para_dims_; ++i) {
    vars_[i] = old_vars_[i] + a * step_[i];
  }
}
void MultiMLMREML::CalcRes(double *Res) {
  for (size_t l = 0; l < num_samples_; ++l) {
    auto eigen = lambda_[l];
    auto *tmp_H = H + l * num_dims_ * num_dims_;
    auto *tmp_r = Ht + l;
...
void MultiMLMREML::GetSigmaE(double *sigma_e) {
  for (size_t i = 0; i < num_dims_; i++) {
    for (size_t j = i; j < num_dims_; j++) {
...
void MultiMLMREML::GetSigmaG(double *sigma_g) {
  for (size_t i = 0; i < num_dims_; i++) {
    for (size_t j = i; j < num_dims_; j++) {
