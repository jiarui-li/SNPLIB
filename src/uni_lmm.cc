...
double UniLMM::CalcLineGradient() {
  return gradients_[0] * step_[0] + gradients_[1] * step_[1];
}
void UniLMM::CalcInitialGuess(const double *trait) {
  trait_ = trait;
...
void UniLMM::BackupVars() {
  old_vars_[0] = vars_[0];
  old_vars_[1] = vars_[1];
}
void UniLMM::UpdateVars(double a) {
  vars_[0] = old_vars_[0] + a * step_[0];
  vars_[1] = old_vars_[1] + a * step_[1];
}
void UniLMM::CalcRes(double *res) {
  for (size_t i = 0; i < num_samples_; ++i) {
    res[i] = t[i] * h[i] * h[i];
  }
}
void UniLMM::CalcBeta(double *beta) {
  double v_e = vars_[0] * vars_[0];
  double v_g = vars_[1] * vars_[1];
  for (size_t i = 0; i < num_samples_; ++i) {
    h[i] = v_e + lambda_[i] * v_g;
