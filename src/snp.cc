...
const std::array<double, 4> kGenoTable = {0.0, 0.0, 1.0, 2.0};
const std::array<double, 4> kMaskTable = {1.0, 0.0, 1.0, 1.0};
void SNP::Unpack(double *geno_d, double *mask_d) const {
...
void SNP::Unpack(double *geno_d,
                 const std::array<double, 4> &geno_table) const {
  for (size_t i = 0; i < num_full_bytes_; ++i) {
    auto t = geno_[i];
