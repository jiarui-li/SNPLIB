...
def CalcUniLMM(traits, covariates, relationship_matrix, num_threads):
    _, D, VT = npl.svd(relationship_matrix)
    Std = np.std(traits, axis=0, ddof=1)
    Y = np.matmul(VT, stats.zscore(traits, axis=0, ddof=1))
    X = np.matmul(VT, covariates)
    Vars, Res = lib.CalcUniLMM(Y, X, D, num_threads)
    return Vars, Res


def CalcMLMMSigmas(traits, covariates, relationship_matrix, num_dims, num_threads):
    _, D, VT = npl.svd(relationship_matrix)
    Std = np.std(traits, axis=0, ddof=1)
    Y = np.matmul(VT, stats.zscore(traits, axis=0, ddof=1))
    X = np.matmul(VT, covariates)
    Vars, Res = lib.CalcUniLMM(Y, X, D, num_threads)
    Sigma_e, Sigma_g = lib.CalcMLMMSigmas(
        Y, X, D, Res, Vars, num_dims, num_threads)
    return Sigma_e, Sigma_g


def CalcRMLMMSigmas(traits, covariates, relationship_matrix, num_dims, num_threads):
    _, D, VT = npl.svd(relationship_matrix)
    Std = np.std(traits, axis=0, ddof=1)
    Y = np.matmul(VT, stats.zscore(traits, axis=0, ddof=1))
    X = np.matmul(VT, covariates)
    Vars, Res = lib.CalcUniLMM(Y, X, D, num_threads)
    Sigma_e, Sigma_g = lib.CalcRMLMMSigmas(
        D, Res, Vars, covariates.shape[1], num_dims, num_threads)
    return Sigma_e, Sigma_g
